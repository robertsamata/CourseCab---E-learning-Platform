/* eslint-disable react/prop-types */
import { useState, useContext } from "react";
import { Dialog } from "@headlessui/react";
import Cookies from "js-cookie";
import { jwtDecode } from "jwt-decode";
import SwitchToggle from "../SwitchToggle";
import { UserContext } from "./Context";
import { toast } from "react-toastify";

function LoginView({ isLogin, changeAuthView }) {
  const [userInput, setUserInput] = useState("");
  const [passwordInput, setPasswordInput] = useState("");
  const { setAccessInfo } = useContext(UserContext);

  const sendLogin = async () => {
    try {
      const loginCredentials = { username: userInput, password: passwordInput };
      const loginResponse = await fetch("/api/login", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(loginCredentials),
      }).then(async (res) => await res.json());

      if (loginResponse.status === 401) {
        setUserInput("");
        setPasswordInput("");
        return toast.error(loginResponse.message);
      }

      const token = loginResponse.token;
      Cookies.set("token", token);
      const decodedToken = jwtDecode(token);
      setAccessInfo(decodedToken.sub);

      toast.success("Login successful!");
    } catch (error) {
      toast.error("An error has occured in the login process. Please try again");
    }
  };

  return (
    <Dialog.Panel className="bg-white text-black rounded-lg px-6 py-4 w-1/6">
      <div className="w-full flex justify-center">
        {" "}
        <SwitchToggle isLogin={isLogin} onChange={changeAuthView} />{" "}
      </div>
      <Dialog.Title className="text-2xl font-bold">Log in</Dialog.Title>

      <div className="flex flex-col space-y-2 items-left py-6">
        <div className="w-full flex flex-col space-y-1">
          <label> Username </label>
          <input
            onInput={(e) => setUserInput(e.target.value)}
            value={userInput}
            className="bg-white w-full border border-black px-2 rounded-md"
            type="text"
          />
        </div>
        <div className="w-full flex flex-col space-y-1">
          <label> Password </label>
          <input
            onInput={(e) => setPasswordInput(e.target.value)}
            value={passwordInput}
            className="bg-white w-full border border-black px-2 rounded-md"
            type="password"
          />
        </div>
        <div className="pt-6 flex w-full justify-center">
          <button
            onClick={() => sendLogin()}
            type="button"
            className="bg-blue-500 rounded-lg w-20 h-8 text-white text-lg font-medium"
          >
            Log in
          </button>
        </div>
      </div>
    </Dialog.Panel>
  );
}
export default LoginView;
